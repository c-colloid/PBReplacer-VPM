...
1. **Tools > PBReplacer**（または Hierarchy の右クリック > PBReplacer）でウィンドウを開く
2. 左のノードにアバターをドロップ ①（クリックして選ぶこともできます）
3. 真ん中の **再配置 n** を押す ②（n は未処理の件数）

![使い方](Docs~/images/usage-steps.png)

再配置が終わると流れが緑になり、行の ○ が ✔ に変わります。Ctrl+Z か ↶ で元に戻せます。
...
| # | 部位 | 操作 |
|---|---|---|
| ① | ツール行 | ↻ 再読み込み / ↶ 元に戻す / ⚙ 詳細設定 / ⋮ その他（PBRemap への導線など） |
| ② | 流れ | 線の色が状態。琥珀 = 未処理あり、緑 ✔ = すべて配置済み、赤 = エラー（Console に詳細） |
| ③ | レール | カテゴリのアイコン。数字が未処理の件数。クリックで列の表示切替、Alt+クリックでその列だけ表示 |
| ④ | 列 | ○ 未処理 / ✔ 配置済み（列末尾に畳めます）。行にマウスを乗せると「Hierarchy で表示」「削除」 |
| ⑤ | ＋ | Hierarchy のオブジェクトを列へドロップすると、そのカテゴリのコンポーネントを追加 |
