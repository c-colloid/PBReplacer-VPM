...
SceneView では青の輪（移植元）→ 緑の点（移植先）を線で結びます。対応先が無い骨の直し方は次の節を参照してください。

#### うまくいかないとき（対応先が見つからない骨がある）

移植先に同じ名前の骨が無いと、その参照は ✖（未解決）になります。同じ名前の骨が 2 つあるときは ▾（要選択）です。移植は実行できますが、未解決の参照は移植元を指したまま残ります。
...
| ② | 表の赤い行（None） | 右の欄へ移植先の骨を Hierarchy からドロップ。▾ の行は ▾ から候補を選択 |

SceneView では 👁 を押すと対応線が出て、未解決の骨は赤い ✕ で示されます。クリックで「ボーン対応」ツールが始まり、マウスに追従する点線で「どの骨と結ぶか」を決められます。

![ボーン対応ツール](Docs~/images/remap-tool-steps.png)

| # | 操作 | 画面 |
|---|---|---|
| ① | 移植元の赤い ✕ をクリック | ツールが始まり、その骨から点線がマウスに追従します |
| ② | 移植先の骨にマウスを重ねる | 骨に白い点が出ます。重ねた骨は緑になり名前が表示されます |
| ③ | クリックで確定 | 青い線で結ばれ、次の未解決の骨へ自動で進みます。Esc で終了 |

対応が決まったら Inspector の **移植 ▶** を押します。Hierarchy の AvatarDynamics 行には状態アイコンが出ます（▶ 移植できる / 🔗 接続済み / 参照切れ）。
